...
export async function startHttpServer(opts: {
  ...
  const handle = async (req: express.Request, res: express.Response) => {
    ...
    try {
      ...
      if (req.method === 'POST') {
        ...
        const server = createKrabbxMcpServer(opts.client);
  ...
  app.get('/health', (_req, res) => {
    res.json({ status: 'ok', service: 'krabbx-mcp' });
  });

  await new Promise<void>((resolve) => {
    app.listen(opts.port, opts.host, () => {
      console.error(`krabbx-mcp Streamable HTTP listening on http://${opts.host}:${opts.port}/mcp`);
      resolve();
    });
  });
}
