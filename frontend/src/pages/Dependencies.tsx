...
import { dependencyApi, repositoryApi, type DependencyFilters } from '../services/api';
import { getUpdateTypeColor, getDependencyTypeColor, getDependencyTypeLabel, getDependencyTypeIcon, cn } from '../lib/utils';
...
export function Dependencies() {
  ...
  const filters: DependencyFilters = {
    ...
    limit: 30,
    isOutdated: (searchParams.get('isOutdated') as 'true' | 'false' | 'all') || 'all',
    updateType: (searchParams.get('updateType') as DependencyFilters['updateType']) || 'all',
    search: searchParams.get('search') || undefined,
    sortBy: (searchParams.get('sortBy') as DependencyFilters['sortBy']) || 'packageName',
  ...
  const showEmptyState = !isLoading && data?.pagination.total === 0 && !scan.isScanning && !filters.search && filters.isOutdated === 'all' && filters.updateType === 'all';
  ...
  return (
    <div className="space-y-6 relative">
          ...
          <div className="text-center max-w-md px-6">
            <div className="mb-6">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-6">
                Start by scanning your organization to discover dependencies and check for outdated packages.
              </p>
            </div>
            <button
              type="button"
      ...
      <div className={cn('flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4', showEmptyState && 'pointer-events-none opacity-50 blur-sm')}>
        <div>
        ...
        <div className="flex gap-2">
          <div className="px-3 py-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-lg">
            <p className="text-xs text-amber-600 dark:text-amber-400 font-semibold uppercase tracking-wide leading-none">Outdated</p>
            <p className="text-lg font-bold text-amber-600 dark:text-amber-400 leading-tight mt-0.5">{stats?.outdated || 0}</p>
          </div>
          <div className="px-3 py-2 bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-700 rounded-lg">
            <p className="text-xs text-primary-600 dark:text-primary-400 font-semibold uppercase tracking-wide leading-none">Open PRs</p>
            <p className="text-lg font-bold text-primary-600 dark:text-primary-400 leading-tight mt-0.5">{stats?.withOpenPR || 0}</p>
          </div>
        </div>
      </div>

      {/* Filters */}
      <div className={cn('card p-4', showEmptyState && 'pointer-events-none opacity-50 blur-sm')}>
      ...
      {/* Table */}
      <div className={cn('card overflow-hidden', showEmptyState && 'pointer-events-none opacity-50 blur-sm')}>
              ...
              {isLoading ? (
                ...
                data?.data.map((dep) => (
                  <tr key={dep.id}>
                    ...
                    <td>
                      {dep.dependencyType && (
                        <span className={`${getDependencyTypeColor(dep.dependencyType)} flex items-center gap-1 w-fit`}>
                          {iconMap[getDependencyTypeIcon(dep.dependencyType)]}
                          {getDependencyTypeLabel(dep.dependencyType)}
                        </span>
                      )}
                    </td>
                    <td className="font-mono text-sm">{dep.currentVersion}</td>
                    <td className="font-mono text-sm">{dep.latestVersion || '-'}</td>
                    <td>
                      {dep.updateType && (
