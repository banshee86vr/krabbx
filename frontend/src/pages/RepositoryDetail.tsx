...
import { cn, formatDateTime, formatRelativeTime, getDependencyTypeColor, getDependencyTypeLabel, getDependencyTypeIcon } from '../lib/utils';
  ...
  if (!repo) {
    return (
      <div className="text-center py-12">
        ...
        <Link to="/repositories" className="text-primary-600 hover:text-primary-700 mt-2 inline-block">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 mt-3 text-sm text-gray-500">
                <span className="flex items-center gap-1">
                  ...
                  Last scan: {repo.lastScanAt ? formatRelativeTime(repo.lastScanAt) : 'Never'}
                </span>
          ...
          <div className="flex flex-col items-end gap-4">
            <div className="text-right">
              <p className="text-sm text-amber-600 font-medium uppercase tracking-wide">Outdated Dependencies</p>
              <p className="text-3xl font-bold text-amber-600 mt-1">{repo.outdatedDependencies}</p>
            </div>
            <div className="flex items-center gap-3">
              <button
                type="button"
              ...
              <div key={scan.id} className="px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={cn(
                    ...
                    scan.status === 'completed' ? 'bg-green-100' : 'bg-gray-100'
                  )}>
  ...
  return (
    <div className="card">
      <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-900">Open Renovate PRs</h2>
        {prsData && (
      ...
      {isLoading ? (
                ...
                <tr key={`pr-${pr.number}`}>
                  ...
                  <td>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-sm">{pr.packageName || '—'}</span>
                    </div>
                  </td>
                  <td>
                    {pr.dependencyType ? (
                      <span className={`${getDependencyTypeColor(pr.dependencyType)} flex items-center gap-1 w-fit`}>
                        {iconMap[getDependencyTypeIcon(pr.dependencyType)]}
                        <span className="text-xs">{getDependencyTypeLabel(pr.dependencyType)}</span>
                      </span>
                    ) : (
                  ...
                  <td>
                    {pr.updateType ? (
                      <span className={cn(
                        ...
                        (pr.updateType === 'digest' || pr.updateType === 'pin') && 'badge-info'
                      )}>
                        {pr.updateType}
                      </span>
                    ) : (
        ...
        <p className="px-6 py-8 text-center text-red-500">
          Error loading open PRs: {error instanceof Error ? error.message : 'Unknown error'}
        </p>
      ) : (
